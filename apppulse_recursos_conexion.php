...
foreach($apps as $key){
	...
	$query_global = "select
					...
					from 
						resources_resourcesVersionsUsageAndDomains_networkUsage_networkTypeDistributionBean_networkTypePercentageList
					where
	...
	while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
		...
		$arreglodatos["type"]= $objgraf["type"];
		$arreglodatos["value"]= floatval($objgraf["value"]) ;
