...
$query_global = "select 
					appVersion,
					sum(convert(float,affectedUsersRatio)) as affectedUsersRatio
				from
					stability_stabilityGetAllCrashesDetailsData_crashRateByVersion_crashRateBeanList
				where
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["appVersion"]= $objgraf["appVersion"];
	$arreglodatos["affectedUsersRatio"]= floatval($objgraf["affectedUsersRatio"]) ;
