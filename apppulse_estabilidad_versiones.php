...
$query_global = "select top 10
					versions,
					sum(convert(int,numOfCrashes)) as numOfCrashes
				from stability_stabilityGetAllCrashesActionsData_actionsCrashes_actionCrashesList
				where 
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["versions"]= $objgraf["versions"];
	$arreglodatos["numOfCrashes"]= intval($objgraf["numOfCrashes"]) ;
