...
$query_global = "select top 20
					deviceName, 
					avg(convert(float,averageResponseTimeInMS)) as averageResponseTimeInMS,
					sum(convert(int,numberOfUniqueUsers)) as numberOfUniqueUsers
				from 
					performance_deviceOs_performanceGetAllOsAndDeviceData_devicesAndOss_deviceList
				where 
					timestamp>='".$startdate->format('Y-m-d')."' and 
					timestamp<'".$finishdate->format('Y-m-d')."' and
					appId in ('".$appsids."')					
				group by deviceName
				order by numberOfUniqueUsers desc";	
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["name"]= $objgraf["deviceName"];
	$arreglodatos["averageResponseTimeInMS"]= floatval($objgraf["averageResponseTimeInMS"]) ;
