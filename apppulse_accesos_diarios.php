...
foreach($apps as $key){
	...
	$query_global = "select sq1.timestamp, sq1.fundex,sq2.actionPenalty,sq2.launchPenalty,sq3.batteryPenalty,sq3.networkPenalty,sq4.crashPenalty,sq4.errorsPenalty 
						from 
				    (select  convert(varchar(10),timestamp,120) as timestamp, avg(convert(int,fundex)) as fundex 
					 ...
					 group by convert(varchar(10),timestamp,120)) as sq1
						inner join 
					(select  convert(varchar(10),timestamp,120) as timestamp, avg(convert(int,actionPenalty)) as actionPenalty, avg(convert(int,launchPenalty)) as launchPenalty 
					 ...
					 on sq1.timestamp =sq2.timestamp
						inner join
					(select  convert(varchar(10),timestamp,120) as timestamp, avg(convert(int,batteryPenalty)) as batteryPenalty, avg(convert(int,networkPenalty)) as networkPenalty 
					 from fundex_fundexSummary_resourcesPenalty 
					 where timestamp >='".$startdate->format('Y-m-d')."' and timestamp <'".$finishdate->format('Y-m-d')."' and appId='".$key['id']."' 
					 group by convert(varchar(10),timestamp,120)) as sq3
					on sq1.timestamp=sq3.timestamp 
						inner join 
					(select  convert(varchar(10),timestamp,120) as timestamp, avg(convert(int,crashPenalty)) as crashPenalty, avg(convert(int,errorsPenalty)) as errorsPenalty 
	...
	while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
		...
		$arreglodatos["actionPenalty"]= intval($objgraf["actionPenalty"]) ;
		$arreglodatos["launchPenalty"]= intval($objgraf["launchPenalty"]);
		$arreglodatos["batteryPenalty"]= intval($objgraf["batteryPenalty"]);
		$arreglodatos["networkPenalty"]= intval($objgraf["networkPenalty"]);
		$arreglodatos["crashPenalty"]= intval($objgraf["crashPenalty"]);
		$arreglodatos["errorsPenalty"]= intval($objgraf["errorsPenalty"]);
		$arreglodatos["perdido"]= intval($objgraf["errorsPenalty"]) + intval($objgraf["launchPenalty"]) + intval($objgraf["batteryPenalty"]) + intval($objgraf["networkPenalty"]) + intval($objgraf["crashPenalty"]) + intval($objgraf["errorsPenalty"]); ;
