...
$query_global = "select top 20
					deviceModel,
					sum(convert(float,affectedUsers)) as affectedUsers,
					sum(convert(float,totalUsers)) as totalUsers
				from
					stability_stabilityGetAllCrashesDetailsData_crashesDevices_deviceCrashesBeans
				where 
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["name"]= $objgraf["deviceModel"];
	$arreglodatos["affectedUsers"]= floatval($objgraf["affectedUsers"]) ;
