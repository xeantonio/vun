...
$query_global = "select top 20
					deviceName,
					avg(convert(float,batteryUsagePerMinute)) as batteryUsagePerMinute,
					sum(convert(float,numberOfUniqueUsers)) as numberOfUniqueUsers
				from
					resources_resourcesGetOverviewDevicesAndOss_devicesAndOss_deviceList
				where 
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["batteryUsagePerMinute"]= floatval($objgraf["batteryUsagePerMinute"]) ;
