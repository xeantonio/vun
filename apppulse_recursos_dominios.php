...
$query_global = "select domain,convert(decimal(10,2),(total_domain/total)* 100) as porc from
				(
				select
					domain,sum(convert(float,cellularData))as total_domain,
					(
						select
							sum(convert(float,cellularData))
						from
							resources_resourcesVersionsUsageAndDomains_domainsBreakdown_domainsDataList
						where
	...
	while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
		$arreglodatos["domain"]= $objgraf["domain"];
		$arreglodatos["porc"]= floatval($objgraf["porc"]);
