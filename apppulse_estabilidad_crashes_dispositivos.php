...
$query_global = "select top 15
					deviceModelId,
					deviceVendor,
					deviceModel,
					sum(convert(int,affectedUsers)) as affectedUsers
				from
				...
				group by
					deviceModelId,deviceModel,deviceVendor
				order by
					affectedUsers desc";		
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["deviceModel"]= $objgraf["deviceModel"];
