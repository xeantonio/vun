...
foreach($apps as $key){
	...
	$datos['id'] = $key['id'];
	$queryultimo= "select convert(varchar(10),timestamp,120) as timestamp, sum(convert(int,activeUsers)) as activeUsers, sum(convert(int,launches)) as launches from usage_usersSummary_usersOverTime where timestamp >='".$startdate->format('Y-m-d H:i:s')."' and timestamp <'".$finishdate->format('Y-m-d H:i:s')."' and appId='".$key['id']."' group by convert(varchar(10),timestamp,120)";
	//print $queryultimo."<br>";
	//$queryultimo= "select countryCode, sum(convert(int,totalLaunches)) as totalLaunches from locations_geoLocations_usersDurationBuckets where timestamp >='".$startdate->format('Y-m-d H:i:s')."' and timestamp <'".$finishdate->format('Y-m-d H:i:s')."' and appId='".$key['id']."' group by countryCode order by  countryCode asc";
	...
	while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
		...
		$arreglodatos["activeUsers"]= intval($objgraf["activeUsers"]);
		$arreglodatos["launches"]= intval($objgraf["launches"]);
