...
if(empty($_GET['paises'])){
    $paises =  10 ; // default value
}else{
    $paises = intval($_GET['paises']);
}
...
//foreach($apps as $key){
	...
	//$datos['id'] = $key['id'];
$query_global = "select * from 
				(
				select * , row_number() over(partition by timestamp order by convert(varchar(10),timestamp,120) DESC, totalLaunches DESC) AS rk from
					(
					select 
						convert(varchar(10),timestamp,120) as timestamp,
						countryCode,
						sum(convert(int,totalLaunches)) as totalLaunches
					from 
						locations_geoLocations_usersDurationBuckets
					where
						timestamp >='".$startdate->format('Y-m-d')."' and
						timestamp <'".$finishdate->format('Y-m-d')."' and
						appId in ('".$appsids."')
					group by countryCode, convert(varchar(10),timestamp,120)
					--order by timestamp, totalLaunches desc
					) as q
				group by countryCode,totalLaunches,timestamp
				)as w
				where rk<= ".$paises."
				order by timestamp,totalLaunches desc";
...
while($objgraf = sqlsrv_fetch_array($SqlIDultimo)){
	...
	$arreglodatos["countryCode"]= $countries[$objgraf["countryCode"]];
	$arreglodatos["totalLaunches"]= intval($objgraf["totalLaunches"]);
	$arreglodatos["rk"]= intval($objgraf["rk"]);
	
	


	//$arreglodatos["attr"]= array("ts" => array("rowspan" => 2));
