...
foreach($apps as $key){
	...
	$datos['id'] = $key['id'];
	$queryultimo="SELECT timestamp ,fundex, fundexStatus  from fundex_fundexOverTime_fundexOverTimeList where timestamp >= '".$startdate->format('Y-m-d H:i:s')."' and timestamp < '".$finishdate->format('Y-m-d H:i:s'). "' and appId = '".$key['id']."' order by timestamp asc";
